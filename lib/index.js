...
var currentDir = require('./utils/config-manager').currentDir;
	...
	fs.readFile(srcPath, 'utf8', function (err, data) {
		if (err) {
			...
			fse.outputFile(destPath, generatedData)
				.then(data => {
					console.log(chalk.green.bold('Output File generated : ') + destPath);
				})
				.catch(err => {
...
function generateFiles(derivedAnswer) {
	...
	const templateFolderPath = path.resolve(currentDir, configManager.getNodeValue('rootSourceFolder'));
	const rootTemplateDirectory = fse.pathExistsSync(templateFolderPath) ? currentDir : __dirname;
	...
	copyTpl(
		...
		path.resolve(currentDir,
			...
			(!!derivedAnswer.isStateLess ?
				configManager.getNodeValue('componentStateLess.target') :
	...
	if (!!derivedAnswer.isPage) {
		...
		var combinedReducerDestFilePath = path.resolve(currentDir,
													configManager.getNodeValue('rootDestFolder'),
		...
		fs.readFile(combinedReducerDestFilePath, 'utf8', function (err, data) {
			...
			fse.outputFile(combinedReducerDestFilePath, combinedReducerContent)
					.then(data => {
						console.log(chalk.green.bold('Output File generated : ') + combinedReducerDestFilePath);
					})
					.catch(err => {
						console.error(chalk.red(err));
					})
